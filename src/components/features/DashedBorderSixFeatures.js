...
const Card = styled.div`
  ...
  .imageContainer {
    ${tw`border-2 border-indigo-500 text-center rounded-full p-6 flex-shrink-0 relative`}
    img {
      ${tw`w-8 h-8`}
    }
  }
  ...
  .title {
    ${tw`mt-2 font-bold text-xl leading-none text-indigo-500`}
  }
  ...
  return (
    <Container>
      <ThreeColumnContainer>
        <Heading>Our Professional <span tw="text-indigo-500">Services</span></Heading>
        {cards.map((card, i) => (
          <Column key={i}>
            <Card>
